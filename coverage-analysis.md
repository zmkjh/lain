...
本文首次对中国异构 ISP NAT 环境下无基础设施直接 P2P 连通性进行了系统分析。通过结合 IPv6 非对称发起（含 GLOBAL 层级双向发起）和 IPv4 STUN 打洞，**随机用户对的 97.1% 可实现不依赖任何中心化基础设施（唯一外部依赖为 STUN）的直接双向通信**。蒙特卡洛不确定性分析（N=10,000）显示 95% 置信区间为 [96.6%, 99.1%]，最坏扰动场景下覆盖率 94.9%。

本文的核心工程贡献是将 IPv6"前缀不完整"作为独立变量纳入覆盖模型——明确区分了"IPv6 栈在线"（层级 0-1）、"前缀已分配"（层级 2）、"可用于 P2P"（层级 3）四个层次——这是 P2P 库设计中关键但常被忽视的区别。GLOBAL 层级的双向发起机制（双方均可发起，回复流量经状态条目通行）将 IPv6 层覆盖率从仅计算 REACHABLE 的 ~84% 提升至 91.3%。

依层次分解：
1. **IPv6 单独覆盖 91.3%**——含非对称发起和 GLOBAL 双向发起。
2. **STUN 打洞增加 5.8 个百分点至 97.1%**——覆盖大部分 IPv4-only 且至少一方 Cone NAT 的场景。
3. **P2P 中继（任意 Cone/IPv6 节点）覆盖剩余 2.9%**——无需专用服务器。
